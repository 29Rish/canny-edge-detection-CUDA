...
__global__ void gaussianBlur_memlock(float* kernel, unsigned char* source, unsigned char* target, int width, int height) {
...
__global__ void grayscale_memlock(unsigned char* rgbData, unsigned char* grayData, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void intensityGradient_memlock(unsigned char* img, int width, int height, float* magnitudes, float* directions) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void hysteresis_memlock(unsigned char* img, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if (x >= width || y >= height) return;
...
# Retrieve kernels
gaussian_blur = mod.get_function("gaussianBlur_memlock")
grayscale = mod.get_function("grayscale_memlock")
hysteresis = mod.get_function("hysteresis_memlock")
intensity_gradient = mod.get_function("intensityGradient_memlock")
